...
const style = {
  ...
  link: {color: '#888', marginTop: '5px', fontSize: '.8rem'},
  linkHov: {color: '#0070c9', marginTop: '5px', fontSize: '.8rem', textDecoration: 'underline'},
  post: {padding: '1rem', backgroundColor: '#fff'},
    ...
    return (
      <div style={style.post} onMouseEnter={()=>{this.setState({hov:true})}} onMouseLeave={()=>{this.setState({hov:false})}}>
        ...
        <div style={{display: 'inline-block'}}>
          <div><a style={{color: '#888'}} href=''>Loading...</a></div>
          <div style={style.link}>loading.com</div>
          <div style={{color: '#888', fontSize: '.8rem', marginTop: '5px'}}>0 points by <a style={{color: '#888'}} href={`https://news.ycombinator.com/user?id=`}>doot</a> 3 hours ago | 0 comments</div>
        </div>
      </div>
    )
  }
