...
const style = {
  ...
  post: {backgroundColor: '#fff', fontSize: '.9rem', lineHeight: '1.3', borderBottom: '1px solid rgba(0,0,0,.05)', borderLeft: '1px solid rgba(0,0,0,.05)', marginLeft: '7rem'},
    ...
    if (this.state.fetched) {
      ...
      if (p.kids) {
        kids = <span style={this.state.hide ? style.hide : style.show}>{p.kids.slice(0,this.state.limit).map((po, i) => {
            return <Comment9 key={po} pid={po} i={i+1} />
          })}</span>
      }
